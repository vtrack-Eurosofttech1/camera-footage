...
Device.prototype.setfirstdata = function (value) {
  this.firstdata = value;
};

Device.prototype.getfirstdata = function () {
...
const updatebufferJSONFile = (filePath, data) =>{
...
//     //   data = data.slice(0, -1);  // Remove the last character (the comma)
//     //   return data
//     // }
//     // else {
//     //     return data
...
//console.log("timestamp",timestamp)
  ...
  let packagescnt ;
    ...
    const queryHex = query.toString('hex').match(/.{1,2}/g).join(',');
...
try {
  fs.writeFile(path.join(__dirname, 'serverData2new.json'), JSON.stringify(packetData, null, 2), (err) => {
    if (err) {
...
const packetData = {
  length: query.length,  // Length of the buffer in bytes
  data: queryHex  ,       // Comma-separated hex string
  time: Date.now() 
};
...
try {
  ...
  console.log("Error writing to file:", error);
          ...
          console.log("dev",device_info.getDeviceDirectory(),device_info.getDeviceDirectory().split("/"),device_info.getDeviceDirectory().split("/").pop() )
          let newData = {
          ...
          console.log("asas", newData)
          updateJSONFile(newData, filePath)
        //   let filePath1 = path.join(__dirname, device_info.getDeviceDirectory(), `${timestamp}buffer` + '.json');
        //   let da = ''
        //   updatebufferJSONFile(filePath1, da)
        /*   let filePath1 = path.join(__dirname, device_info.getDeviceDirectory(), `${timestamp}buffer` + '.json');
          let da = ''
          updatebufferJSONFile(filePath1, da) */
    ...
    //  console.log("metadata_string",metadata_string)
      ...
      //  console.log("metadata_string",metadata_string)
        try {
          ...
          let  file  = path.join(__dirname, device_info.getDeviceDirectory(), `${timestamp}meta` + '.json');
      // console.log("ascd", jsonObject)
          //  fs.writeFileSync(file, jsonObject)
          fs.writeFileSync(file, JSON.stringify(jsonObject, null, 2), 'utf-8');
          //  dbg.log("[METADATA]: " + metadata_string);
        } catch (error) {
         // dbg.logAndPrint("Error parsing METADATA: " + error);
        }
      } else {
      console.log("ascd", raw_data)
    ...
    DeviceModel.model
      ...
      .then(async (c) => {
        ...
        if(c.length>0){
            ...
            clientId = c[0].deviceassigns.clientId
            vehicle = c[0].vehicles.vehicleReg
        //    console.log(c[0].deviceassigns.clientId,c[0].vehicles.vehicleReg )
