...
MetaData.prototype.setTriggerTimestamp = function (timestamp_ms) {
    ...
    this.trigger_timestamp =
        date.toISOString().replace(/[TZ]/g, " ") +
        "(" +
        timestamp_ms.readBigUInt64BE(0).toString(10) +
        ")";
};
...
//     }
//     let form_command =
//         "ffmpeg -hide_banner -loglevel quiet -r " +
//         framerate +
//         ' -i "' +
//         directory +
//         "\\" +
//         filename +
//         extension +
//         '" -ss 00:00:0.9 -c:a copy -c:v libx264 "' +
//         directory +
//         "\\" +
//         filename +
//         '.mp4"';
...
//     });
// }
function updateReceivedPackages(jsonFilePath) {
    // Read the JSON file
  ...
  ////console.log(jsonData);
        // Ensure the target package exists and update its field
        if (jsonData) {
          jsonData.receivedPackages += 1;
          ...
          fs.writeFile(jsonFilePath, JSON.stringify(jsonData, null, 2), (writeErr) => {
            if (writeErr) {
              ...
              //console.log(`Updated receivedPackages for successfully!`);
            }
          });
        } else {
          console.error(`Package ID not found or receivedPackages is not a number.`);
        }
      } catch (parseErr) {
...
try {
  ...
  current_state = FSM_STATE.WAIT_FOR_CMD;
        ...
        case CMD_ID.DATA: {
         ...
         //   //console.log("crc",computed_crc,actual_crc,data_len)
        //    dbg.log("CRC = Computed: " + computed_crc + ", Actual : " + actual_crc);
            ...
            if (data_len ==null) {
               ...
               // current_state = FSM_STATE.REPEAT_PACKET; 
                ...
                // );
                let buffer = Buffer.from(raw_file, "base64"); // Convert the raw base64 data to a buffer
                console.log("F", buffer.slice(0, 10));
                let filePathUpdate;
                try {
  ...
  // Append the buffer to the file (creating the file if it doesn't exist)
...
console.log("errr", e);
            ...
            updateReceivedPackages(filePathUpdate);
            device_info.incrementReceivedPackageCnt(1);
